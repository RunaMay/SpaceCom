...
public class EstimatedHoursField extends GenericTextCFType {
	 ...
	 * @param jiraBaseUrls
	 */
	public EstimatedHoursField(PluginLicenseManager licenseManager,
	...
	public boolean updateField(MutableIssue mutableIssue, CustomField customField, String string, ApplicationUser user){
		...
		if(mutableIssue!=null){
			...
			if(loggedField!=null){
				String logHours = (String) mutableIssue.getCustomFieldValue(loggedField);
...
width = Integer.toString(percent);
		...
		if (updateValidationResult.isValid()) {
			log.error("update validation passed");
			IssueResult updateResult = issueService.update(user,
					updateValidationResult);
			if (updateResult.isValid()) {
				log.error("update result valid");
				try {
	...
	public Map<String, Object> getVelocityParameters(Issue issue,
		...
		String value = "0";
		if(issue!=null){
			value = (String) issue.getCustomFieldValue(PluginUtil.getEstimatedHoursField());
			log.error(value+"-----issue value ESTIM");
